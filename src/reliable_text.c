...
typedef struct {
  ...
  COMP inbound_pending_syms[(RELIABLE_TEXT_UW_LENGTH_BITS + LDPC_TOTAL_SIZE_BITS) /
                            2];
...
static int reliable_text_ldpc_decode(reliable_text_impl_t* obj, char* dest) {
  ...
  if (obj->bit_index == obj->sym_index * 2) {
    ...
    gp_deinterleave_comp(
        deinterleavedSyms,
        &obj->inbound_pending_syms[RELIABLE_TEXT_UW_LENGTH_BITS / 2],
    ...
    symbols_to_llrs(llr, deinterleavedSyms, deinterleavedAmps, EsNo,
